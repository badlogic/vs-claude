...
export class LogViewerWebviewProvider {
	...
	private getWebviewContent(webview: vscode.Webview): string {
		...
		const scriptUri = webview.asWebviewUri(
			vscode.Uri.joinPath(this.context.extensionUri, 'out', 'log-viewer-webview.content.js')
		);

		// Read the HTML template
		const htmlPath = path.join(this.context.extensionPath, 'out', 'log-viewer-webview.content.html');
